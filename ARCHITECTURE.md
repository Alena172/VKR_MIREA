...
Для top-level модулей используется единая HTTP-точка входа `api/router.py`. Крупные модули (`identity`, `learning`, `vocabulary`) дополнительно делятся на подмодули, а компактные модули (`learning_graph`, `ai_services`) сохраняют плоскую внутреннюю структуру.
...
`learning_graph` формирует персональный semantic profile пользователя: накапливает интересы по сохраненным словам и связывает словарные элементы с конкретными смыслами.
...
- простые anchors между смыслами: полисемия или слова из той же темы
