...
## Правило границ модулей

Backend развивается как модульный монолит не только по структуре каталогов, но и по правилам импорта.

Разрешено:
- внутри модуля использовать свои `repository`, `application_service`, `models`, `schemas`
- обращаться к другому модулю через его `public_api`
- использовать общие платформенные компоненты из `app.core.*`
- собирать межмодульные контракты через `contracts.py` и `assembler.py`

Запрещено:
- импортировать `repository` другого модуля
- импортировать `application_service` другого модуля напрямую
- импортировать `models` другого модуля напрямую
- возвращать из `public_api` ORM-модели или HTTP/Pydantic response-схемы другого модуля

Практическое правило:
- если модуль `vocabulary` хочет обновить граф обучения, он вызывает `learning_graph.public_api`
- если модуль `exercise_engine` хочет получить CEFR/SRS-контекст, он вызывает `context_memory.public_api`
- `public_api` возвращает DTO из `contracts.py`, а преобразование делает `assembler.py`

Для контроля этого правила в backend есть автоматическая проверка:
...
- `contracts.py` — межмодульные DTO
- `assembler.py` — преобразование внутренних моделей в DTO
- `router.py` — только HTTP-адаптер и маппинг в response schema
