...
def test_translate_single_word_uses_shared_dictionary_not_ai(client, db):
    ...
    entry, _ = repo.get_or_create_dictionary_entry(
        english_lemma="journey",
        russian_translation="путешествие",
        context_definition_ru=None,
    )
    repo.add_to_user_vocabulary(user_id=user.id, entry_id=entry.id, source_sentence=None, source_url=None)
    db.commit()
    ...
    try:
        ...
        result = asyncio.run(service.translate_for_user(user_id=user.id, text="journey", source_context=None))
    finally:
...
def test_translate_with_context_still_calls_ai(client, db):
    ...
    repo = VocabularyRepository(db)
    entry, _ = repo.get_or_create_dictionary_entry(
        english_lemma="light", russian_translation="свет", context_definition_ru=None
    )
    repo.add_to_user_vocabulary(user_id=user.id, entry_id=entry.id, source_sentence=None, source_url=None)
    db.commit()
    ...
    try:
        service = VocabularyService(repo)
        result = asyncio.run(
            service.translate_for_user(user_id=user.id, text="light", source_context="The bag is very light.")
        )
    finally:
        ai_facade_module.ai_facade.translate_with_context_async = original
...
def test_capture_new_word_already_in_shared_dictionary_skips_ai(db):
    ...
    repo = VocabularyRepository(db)
    entry, _ = repo.get_or_create_dictionary_entry(
        english_lemma="truck", russian_translation="грузовик", context_definition_ru=None
    )
    repo.add_to_user_vocabulary(user_id=user1.id, entry_id=entry.id, source_sentence=None, source_url=None)
    db.commit()
