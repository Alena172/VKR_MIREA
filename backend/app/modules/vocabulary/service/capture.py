...
def _to_capture_dto(uv, entry) -> CaptureDTO:
    return CaptureDTO(vocabulary=VocabularyItemDTO.from_model(uv, entry))
...
async def capture_to_vocabulary(
    *,
    ...
    with application_transaction.boundary(db=db):
        existing_row = repository.get_latest_vocabulary_item_by_lemma(
            db,
            user_id=user_id,
            english_lemma=english_lemma,
        )
        use_existing = existing_row is not None and not capture.force_new_vocabulary_item
        ...
        from app.modules.review.service.srs import srs_service as context_memory_public_api  # noqa: PLC0415
