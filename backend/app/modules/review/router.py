...
from app.modules.vocabulary.service.items import get_translation_map_for_user
...
def get_review_queue_me(
    ...
    result = srs_service.get_review_queue(db=db, user_id=current_user_id, current_user_id=current_user_id, limit=limit)
...
def submit_review_queue_item_me(
    ...
    progress = srs_service.submit_review_queue_item(db=db, user_id=current_user_id, current_user_id=current_user_id, payload=payload)
    translation_map = get_translation_map_for_user(db, user_id=current_user_id, english_lemmas=[progress.word])
...
def submit_review_queue_bulk_me(
    ...
    result = srs_service.submit_review_queue_bulk(db=db, user_id=current_user_id, current_user_id=current_user_id, payload=payload)
    rows = result["updated"]
    words = [r.word for r in rows]
    translation_map = get_translation_map_for_user(db, user_id=current_user_id, english_lemmas=words)
    updated = [
        WordProgressRead(
            user_id=r.user_id,
            word=r.word,
...
def start_review_session_me(
    ...
    result = srs_service.start_review_session(db=db, user_id=current_user_id, current_user_id=current_user_id, payload=payload)
...
def list_word_progress_me(
    ...
    result = srs_service.list_word_progress(
        db=db, user_id=current_user_id, current_user_id=current_user_id,
...
def get_word_progress_me(
    ...
    progress = srs_service.get_word_progress(db=db, user_id=current_user_id, current_user_id=current_user_id, word=word)
    translation_map = get_translation_map_for_user(db, user_id=current_user_id, english_lemmas=[progress.word])
    return WordProgressRead(
        user_id=progress.user_id,
        word=progress.word,
...
def delete_word_progress_me(
    ...
    result = srs_service.delete_word_progress(db=db, user_id=current_user_id, current_user_id=current_user_id, word=word)
...
def get_review_plan_me(
    ...
    result = srs_service.get_review_plan(
        db=db, user_id=current_user_id, current_user_id=current_user_id,
...
def progress_me(
    ...
    result = srs_service.get_progress_snapshot(db=db, user_id=None, current_user_id=current_user_id)
...
def review_summary_me(
    ...
    result = srs_service.get_review_summary(
        db=db, user_id=current_user_id, current_user_id=current_user_id,
