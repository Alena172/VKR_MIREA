...
def to_exercise_generate_result_dto(
    *,
    ...
    return ExerciseGenerateResultDTO(exercises=list(exercises), note=note)


def to_word_progress_tracking_dto(*, word: str, tracked: bool) -> WordProgressTrackingDTO:
    return WordProgressTrackingDTO(word=word, tracked=tracked)


def to_learning_progress_update_result_dto(*, updated_words: list[str]) -> LearningProgressUpdateResultDTO:
...
def to_progress_snapshot_dto(*, user_id: int, total_sessions: int, avg_accuracy: float) -> ProgressSnapshotDTO:
    return ProgressSnapshotDTO(user_id=user_id, total_sessions=total_sessions, avg_accuracy=avg_accuracy)
...
def to_review_queue_response_dto(
    *,
    ...
    return ReviewQueueResponseDTO(user_id=user_id, total_due=total_due, items=list(items))
...
def to_word_progress_list_dto(
    *,
    ...
    return WordProgressListDTO(user_id=user_id, total=total, limit=limit, offset=offset, items=list(items))
...
def to_review_queue_bulk_submit_dto(
    *,
    ...
    return ReviewQueueBulkSubmitDTO(user_id=user_id, updated=list(updated))
...
def to_review_session_start_dto(
    *,
    ...
    return ReviewSessionStartDTO(user_id=user_id, mode=mode, total_items=total_items, items=list(items))


def to_word_progress_delete_dto(*, user_id: int, word: str, progress_deleted: bool) -> WordProgressDeleteDTO:
    return WordProgressDeleteDTO(user_id=user_id, word=word, progress_deleted=progress_deleted)
...
def to_learning_progress_dto(*, total_sessions: int, average_accuracy: float) -> LearningProgressDTO:
    return LearningProgressDTO(total_sessions=total_sessions, average_accuracy=average_accuracy)
