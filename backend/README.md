...
Если хостинг запускает FastAPI через автоопределение и пишет, что не может
найти файл по умолчанию, используй директорию `backend/` как корень сервиса и
явный entrypoint `main.py` или команду `uv run uvicorn app.main:app --host
0.0.0.0 --port $PORT`.
