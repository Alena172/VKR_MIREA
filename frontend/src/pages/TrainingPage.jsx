...
export default function TrainingPage({ onError }) {
  ...
  // Прогреваем буфер пока пользователь на странице упражнений — каждые 30 сек
  const warmupControllerRef = useRef(null);
  useEffect(() => {
    function ping() {
      if (warmupControllerRef.current) warmupControllerRef.current.abort();
      warmupControllerRef.current = new AbortController();
      api.generateExercisesMe(
        { size: 10, mode: "sentence_translation_full", vocabulary_ids: [], fast_start: false, incremental: true },
        { signal: warmupControllerRef.current.signal },
      ).catch(() => {});
    }
    ping();
